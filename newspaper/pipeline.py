...
def _score(article, excerpt, chat_model, system_prompt, valid_cats):
    ...
    ok = False
    if not AI_ENABLED:
        return score, summary, category, ok, 0.0
    t0 = time.monotonic()
    try:
    ...
    try:
        ...
        for a in _load_prev_digest_articles():
            ...
            if not aid or aid in read_ids or aid in fresh_ids:
                continue
            # One-time-safe source migration: Reuters now comes from its
            # first-party sitemap with canonical URLs and artwork. Do not carry
            # forward image-less Google News wrappers from the retired feed.
            if (str(a.get('source') or '').casefold() == 'reuters'
                    and urllib.parse.urlparse(str(a.get('url') or '')).hostname
                    == 'news.google.com'):
                continue
        ...
        if AI_ENABLED:
            ...
            log(f'deterministic enrichment: {scored_count} stories (AI disabled)')
