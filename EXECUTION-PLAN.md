...
| 2026-09-03 | Post-v1 roadmap reconciliation | Reconciled the fresh 33-source inventory, accepted v1 boundary, running edition archive, historical expansion stages, and the owner's request to move routine source governance out of Notion. | Confirmed 32 active sources and one genuine gap (Kickstarter). Confirmed production already publishes immutable 07:30/16:30 editions. Removed delivered comics, density, weather, cross-device state, health, and edition machinery from the outstanding list; recast ATP, Reuters, and Medium as optional quality upgrades. Created `V2-ROADMAP.md` with an ordered owner-facing backlog, later expanded to seven priorities. | Begin Priority 1 with editable canonical editorial guidance and a deliberate source-research queue in the Sources interface. Do not reopen completed v1 work or treat working discovery feeds as absent sources. |
| 2026-09-03 | v2 visual-identity options | Gabriel requested that the four visual approaches discussed after reviewing the inherited Cruxwire magazine foundation be retained in the v2 backlog. | Added a dedicated roadmap priority for Modern Magazine, Traditional Broadsheet, Editorial Journal, and Newsroom Dashboard identities. The current accepted presentation remains the default; light/dark support, synchronized selection, shared design tokens, complete-view coverage, and desktop/phone parity are acceptance requirements. | Execute after edition-workflow polish unless Gabriel explicitly reprioritizes it. Prototype all four against the same representative issue before commissioning any selector. |
| 2026-09-03 | Forge browser icon | Gabriel requested a Forge-related page icon after Arc displayed the inherited application emblem. | Replaced the favicon family with a purpose-built hammer-and-anvil mark in the publication palette. Added an SVG primary icon, matching ICO/PNG/Apple touch fallbacks, explicit server/container inclusion, and versioned references to defeat stale browser caches. | Deploy to `4-main-pi`, verify the SVG and fallback assets over the live reader route, and visually confirm the tab-sized mark. |
| 2026-09-04 | Card-density and FT/Reuters artwork repair | Gabriel no longer needed the repeated “Why it’s here” paragraph and reported image-less FT and Reuters cards. | Kept `why_selected` in publication data but removed it from every visible card. FT supplies usable image metadata for many but not all pieces; Reuters' Google discovery items had none. Reuters' public first-party news sitemap provided canonical URLs and artwork for 68 of 72 sampled current world stories. Added a source/path allow-listed same-origin relay for both publishers, a Reuters sitemap parser, and restrained source artwork when a story genuinely has no supplied photograph. | Replace the production Reuters discovery endpoint with bounded first-party sitemap slices, refresh, verify every selected FT/Reuters card renders either its story image or the honest source fallback, then inspect desktop and phone grids for density regressions. |
| 2026-09-04 | v2 Formula 1 race-desk rebuild | Gabriel rejected the six-card F1 page as generic rumor/interview filler with no standings, practice results, technical reporting, or meaningful Motorsport/Autosport presence. | Added cached driver and constructor championship tables from Jolpica and finalized race-weekend timing classifications from Formula 1's public static archive. Expanded the dedicated page to twelve source-balanced and reporting-type-labelled stories while retaining six on Home; protected Formula 1, Motorsport, and Autosport, added the Autosport F1 RSS feed, and capped rumor/interview filler. OpenF1 and live F1 timing were rejected as default dependencies because current live access requires authentication. | Deploy, refresh during the Italian GP weekend, verify the FP1 classification and both championship tables, then inspect all twelve F1 stories plus the six-card Home excerpt on desktop and phone. |
