...
| 2026-09-04 | Card-density and FT/Reuters artwork repair | Gabriel no longer needed the repeated “Why it’s here” paragraph and reported image-less FT and Reuters cards. | Kept `why_selected` in publication data but removed it from every visible card. FT already had valid image metadata, pointing to third-party browser delivery as the fault. Reuters' Google discovery items had no image metadata; Reuters' public first-party news sitemap provided canonical URLs and artwork for 68 of 72 sampled current world stories. Added a source/path allow-listed same-origin relay for both publishers and a Reuters sitemap parser. | Replace the production Reuters discovery endpoint with bounded first-party sitemap slices, refresh, verify selected FT/Reuters cards carry and render images, then inspect desktop and phone grids for density regressions. |
